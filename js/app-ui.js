...
document.getElementById(`resetBtn`).onclick=()=>{pushUndo();if(cur===`trav`){M.trav.routes.forEach(r=>{r.pts.forEach(p=>map.removeLayer(p.marker));});M.trav.impGhosts.forEach(g=>map.removeLayer(g.marker));M.trav.impGhosts=[];M.trav.routes=[];M.trav.activeRouteId=null;}else if(cur===`lev`){M.lev.routes.forEach(r=>{r.pts.forEach(p=>map.removeLayer(p.marker));});M.lev.impGhosts.forEach(g=>map.removeLayer(g.marker));M.lev.impGhosts=[];M.lev.routes=[];M.lev.activeRouteId=null;}else{m().pts.forEach(p=>map.removeLayer(p.marker));m().pts=[];m().impGhosts.forEach(g=>map.removeLayer(g.marker));m().impGhosts=[];if(cur===`gnss`){M.gnss.edges=[];M.gnss.triangles=[];M.gnss.sel=null;M.gnss.triSel=[];}}selectedPtIds.clear();refresh();toast(`已清空当前模式`);};
